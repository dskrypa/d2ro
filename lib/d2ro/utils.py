...
def print_split_hex(data: bytes, per_line: int = 8):
    data_len = len(data)
    pre_fmt = '{:0' + str(len(f'{data_len:x}')) + 'x}  '
    for i in range(0, math.ceil(data_len / per_line)):
        a = i * per_line
        b = a + per_line
        print(to_hex_and_str(pre_fmt.format(a), data[a:b], fill=per_line))
