...
def get_path(path: str = None, character: str = None) -> Path:
    if path:
        path_obj = Path(path).expanduser()
        if not path_obj.exists() and '/' not in path and '\\' not in path:
            path_obj = get_save_dir().joinpath(path)
    elif character:
        path_obj = get_save_dir().joinpath(f'{character.title()}.ctlo')
    else:
        raise ValueError('Either --path / -p or --character / -c are required')
...
def get_save_dir() -> Path:
    save_dir = Path('~/Saved Games/Diablo II Remastered/').expanduser()
    if not save_dir.exists():
        raise PathRequired(f'save_dir={save_dir.as_posix()} does not exist')
    steam_dirs = list(save_dir.iterdir())
    if len(steam_dirs) != 1:
        raise PathRequired(f'a single directory under save_dir={save_dir.as_posix()} does not exist')
    return steam_dirs[0]
